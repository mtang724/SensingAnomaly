...
class GraphDetector(nn.Module):
    def __init__(self, in_channels, conv_ch, dropout, original_dim, num_head, st_module=d3GraphConv2):
        ...
        self.conv3 = st_module(conv_ch // 2 * 3, conv_ch // 2, num_head, dropout=dropout)
        self.conv4 = st_module(conv_ch * 2, conv_ch, num_head, dropout=dropout)
        self.conv5 = st_module(conv_ch, conv_ch // 2, num_head, dropout=dropout)
        ...
        self.reconstruct21 = GATv2Conv(conv_ch, conv_ch, num_head, concat=False)
        self.reconstruct22 = GATv2Conv(conv_ch, in_channels, num_head, concat=False)
        ...
        self.forecast21 = GATv2Conv(conv_ch, conv_ch, num_head, concat=False)
        self.forecast22 = GATv2Conv(conv_ch, in_channels, num_head, concat=False)
    ...
    def forward(self, x, edge):
        ...
        """
        out1 = F.elu(self.conv1(x, edge))
        out2 = F.elu(self.conv2(out1,edge))
        out2 = torch.cat((out2, out1), -1)
        out3 = F.elu(self.conv3(out2,edge))
        out3 = torch.cat((out3, out2), -1)
        out4 = F.elu(self.conv4(out3, edge))
        E = F.elu(self.conv5(out4, edge))
        ...
        recon = F.elu(self.reconstruct21(recon, edge_index))
        recon = torch.tanh(self.reconstruct22(recon, edge_index)) # F.elu(self.reconstruct2(recon))
        ...
        forecast = F.elu(self.forecast21(forecast, edge_index))
        forecast = torch.tanh(self.reconstruct22(forecast, edge_index)) # F.elu(self.forecast2(forecast))
